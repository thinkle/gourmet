...
__version__ = "2.1.1"
__date__ = "$Date: 2008/10/29 02:11:07 $"
...
class BeautifulStoneSoup(Tag, SGMLParser):
    ...
    def handle_charref(self, ref):
        ...
        try:
            ...
            print 'Handled ',ref,'->',str(unichr(int(ref)))
        except ValueError:
            self.handle_data('&#%s;' % ref)

    def handle_entityref(self, ref):
        "Propagate entity refs right through."
        # MODIFIED BY TOM -- WE WANT TO REPLACE ENTITIES
        if htmlentitydefs.entitydefs.has_key(ref):
            print 'handled ',ref,'->',htmlentitydefs.entitydefs[ref].decode('iso8859')
            self.handle_data(htmlentitydefs.entitydefs[ref].decode('iso8859'))
        else:
            self.handle_data('&%s;' % ref)
