import gtk, gtk.glade, gobject, rmetakit, re
...
class KeyEditor:
    ...
    def makeTreeModel (self):
        ...
        for k in unique_key_vw:
            if k.key:
                iter=self.treeModel.append(None,[k, k.key, "",len(k.groupvw),""])
                self.unique_item_vw = k.groupvw.groupby(k.groupvw.item, 'itemgroup')
                cnt = 0
                if not self.key_to_item.has_key(k.key): self.key_to_item[k.key]=[]
                for i in self.unique_item_vw:
                    self.key_to_item[k.key].append(i.item)
