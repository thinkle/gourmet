...
class RecIndex:
	...
	if txt and self.limitButton: self.limitButton.set_sensitive(True)
        ...
        if [txt, searchBy] == self.lsrch:
            debug("Same search!",0)
            return
        self.srchentry.window.set_cursor(gtk.gdk.Cursor(gtk.gdk.WATCH))
    ...
    def do_search (self, txt, searchBy):
        ...
        if txt:
            if searchBy == "ingredient":
                ...
                self.lsrchvw=self.rd.ing_search(txt,rview=self.lsrchvw)
            else:
                self.lsrchvw=self.rd.search(self.lsrchvw,searchBy,txt)
        else:
            self.lsrchvw = self.searchvw
    ...
    def update_rmodel (self, rview):
        debug('update_rmodel... changing filtering criteria')
