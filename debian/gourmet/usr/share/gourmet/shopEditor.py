...
class ShopEditor:
    ...
    def changeItem (self, key, item=None, new_key=None, new_item=None):
        ...
        for i in vw:
            print 'changing item for %s %s %s'%(i.id,i.key,i.item)
            if new_key:
                i.key=new_key
    ...
    def makeTreeModel (self):
        ...
        for c in unique_cat_vw:
            ...
            for i in c.groupvw:
                #self.treeModel.append(iter,[i,pickle.loads(c.category),i.key])
                self.treeModel.append(iter,[i,pickle.loads(c.category),i.key])
                self.cat_to_key[pickle.loads(c.category)].append(i.key)
