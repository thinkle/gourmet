        ...
        'simple recipe': {
            ...
            'ingredients': [
                {'amount': 1, 'unit': 'cup', 'item': 'water',
                    'ingkey': 'water, municipal'},
                {'amount': 2, 'unit': 'cups', 'item': 'atta flour',
                    'ingkey': 'flour, atta (whole wheat)'},
                {'amount': 2, 'unit': 'Tbs', 'item': 'salt', 'ingkey': 'salt, table'},
                {'amount': 1, 'unit': 'tsp', 'item': 'black pepper',
                    'ingkey': 'pepper, black', 'optional': True},
            ],
        },
        'unicode': {
            ...
            'ingredients': [
                {'amount': 1, 'unit': 'cup', 'item': 'water',
                    'ingkey': 'water, municipal'},
                {'amount': 1, 'unit': 'lb', 'item': 'jalapeño',
                    'ingkey': 'pepper, habañero'},
                {'amount': 2, 'unit': 'más', 'item': 'habañeros',
                    'ingkey': 'pepper, habañero'},
            ]
        },
        'formatting': {
    ...
    def testSingleExport(self):
        for format, plugin in list(self.em.plugins_by_name.items()):
            ...
            for rec, f in [(self.db.get_rec(self.recs['simple recipe']['recipe_id']), '/tmp/Simple.'+ext),
                           (self.db.get_rec(
                               self.recs['unicode']['recipe_id']), '/tmp/Uni.'+ext),
                           (self.db.get_rec(
                               self.recs['formatting']['recipe_id']), '/tmp/Formatted.'+ext),
                           ]:
