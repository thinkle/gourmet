...
class RecRenderer (PrintWriter):
    def __init__ (self, recs, rd, prefs, dialog_parent=None, mult=1, dialog_title=None):
        debug('RecRenderer.__init__ ',3)
        if dialog_title is None:
            n = len(recs)
            dialog_title = ngettext('Print %s recipe','Print %s recipes',n)%n
        PrintWriter.__init__(self, show_dialog=True, dialog_title=dialog_title,
                              dialog_parent=dialog_parent)
        do_new_page = False
        for r in recs:
            if do_new_page
                self.new_page
            r=RecWriter(rd, r, self, change_units=prefs.get('readableUnits',True), mult=mult)
