...
def standardize_ingredient (ing_object, conv):
    if ing_object.item:
        ing = ing_object.item
    else:
        ing = ing_object.ingkey
    unit,amount = ing_object.unit,ing_object.amount
    gconv = conv.converter(unit,'g.')
    vconv = conv.converter(unit,'ml.')
    if not (gconv or vconv):
        gconv = conv.converter(unit,'g.',ing)
        vconv = conv.converter(unit,'ml.',ing)
    if gconv:
        unit = 'g.'
...
def hash_recipe (rec, rd, conv=None):
    if not conv: conv = converter.conv
...
if __name__ == '__main__':
    ...
    rd = RecipeManager.default_recipe_manager()
    r1 = 33
    r2 = 241
