...
class Shopper:
    ...
    def combine_ingredient (self, ing, amts):
        ...
        for a,u in amts:
            if not items:
                ...
                flag = 0
                ind = 0
                while not flag and len(items) > ind:
                    amt,unit = items[ind]
                    if type(amt) == tuple or type(a)== tuple :
                        ...
                        add_low = self.converter.add_reasonably(amt[0],unit,a[0],u,ing)
                        add_high = self.converter.add_reasonably(amt[1],unit,a[1],u,ing)
                        if (not add_low) or (not add_high):
                            add = False
                        else:
                            # Adjust units
                            add_low = self.converter.adjust_unit(
                            ...
                            if add_low:
                                add_low = self.converter.adjust_unit(*add_low,**{'favor_current_unit':False})
                            if add_high:
                                add_high = self.converter.adjust_unit(*add_high,**{'favor_current_unit':False})
                            if add_low[1] == add_high[1]: #same unit...
                                add = ((add_low[0],add_high[0]),add_low[1])
                            else:
                                # otherwise, let's use our unit for add_high...
                                u1_to_u2 = self.converter.converter(add_low[1],add_high[1])
                                add = ( (add_low[0]*u1_to_u2,add_high[0]), #amount tuple
                                         add_high[1] #unit from add_high
                                      )
                    else:
                        add = self.converter.add_reasonably(amt,unit,a,u,ing)
                        if add:
                            # adjust unit to make readable
                            add=self.converter.adjust_unit(*add,**{'favor_current_unit':False})
                    # add_reasonably returns a nice a,u pair if successful
                    # Otherwise, it return False/None
                    if add: 
                        items.pop(ind) # out with the old...
                        items.append(add) # in with the new
                        flag = 1
                    else:
                        ind += 1
                if not flag:
    ...
    def amt_to_string (self, amts):
        retstr = self.converter.amt_string(amts[0])
        if len(amts) > 1:
            for a in amts[1:]:
                retstr = "%s + %s" %(self.converter.amt_string(a), retstr)
        return retstr
    ...
    def organize (self, dic=None):
        ...
        for c,d in cats.items():
            ...
            for i,amts in d.items():
                items.append([i,self.amt_to_string(amts)])
            lst.append([c,items])
    ...
    def add_org_itm (self, item, cat):
        self.orgdic[item]=cat
