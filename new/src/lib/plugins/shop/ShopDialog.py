...
#from gourmet.exporters import printer
#from gourmet.nutrition import nutrition, NutritionLabel

from gourmet.gdebug import *
from gourmet.gglobals import *
from gourmet.Shopper import Shopper
from gourmet.WidgetSaver import WidgetSaver, WindowSaver
...
from gourmet.widgets import treeview_extras
from gourmet.widgets.IngredientEditor import IngredientEditor

class ShopDialog (GladeDialog):
    """A class to manage our shopping window."""
    
    glade_file = 'shopList.glade'
    
    def __init__ (self, RecGui):
        debug("__init__ (self, RecGui):",5)
        GladeDialog.__init__(self)
        self.conv = converter.conv
        #self.nd = self.nd # convenient reference to our nutrition
        self.recs = {}
        self.extras = []
        ...
        self.includes = {}
        self.contid = self.widgets['statusbar1'].get_context_id('main')
    ...
    def save (self, *args):
        ...
        self.message('Saving')
        self.do_save(de.select_file(_("Save Shopping List As..."),
    ...
    def do_save (self, filename):
        debug("do_save (self, filename):",5)
        import exporters.lprprinter
        self._print_list(exporters.lprprinter.SimpleWriter,file=filename,show_dialog=False)

    def print_list (self, *args):
        debug("print_list (self, *args):",0)
        self._print_list(printer.SimpleWriter,dialog_parent=self.widget)
        
    def _print_list (self, printer, *args, **kwargs):
    ...
    def grabIngsFromRecs (self, recs, start=[]):
        ...
        #TODO self.sh = recipeManager.mkShopper(lst, self.rg.rd, conv=self.conv)
        self.sh = Shopper(lst)
            ...
            if refid:
                ...
                if subrec:
                    ...
                    if not include_dic.has_key(subrec.id):
                        d = getOptionalIngDic(self.rd.get_ings(subrec),
                                              refmult,
                                              self.prefs)
...
class shopIngredientEditor (IngredientEditor):
    def __init__ (self, RecGui, ShopGui):
        self.sg = ShopGui
        self.last_key = ""
        IngredientEditor.__init__(self, RecGui, None)
