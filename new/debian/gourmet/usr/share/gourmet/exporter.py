...
class exporter_mult (exporter):
    def __init__ (self, rd, r, out, mult=1, conv=convert.converter(),
                  imgcount=1, order=['attr','text','ings'],
                  change_units=True):
        self.mult = mult
...
class mealmaster_exporter (exporter):
    def __init__ (self, rd, r, out, conv=convert.converter()):
...
class html_exporter (exporter_mult):
    def __init__ (self, rd, r, out, conv=convert.converter(), css=os.path.join(gglobals.datad,"default.css"), embed_css=True, start_html=True, end_html=True, imagedir="pics/", imgcount=1, link_generator=None, mult=1):
        ...
        exporter.__init__(self, rd, r, out, conv,imgcount=imgcount)
